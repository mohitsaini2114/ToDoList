//Assignment in class 08 , Name Srishtee Marotkar, Mohit Saini Group 27
...
public class MainActivity extends AppCompatActivity  implements View.OnClickListener,RecyclerViewAdapter.TaskRecycler {
    ...
    public void refreshData(){
        ...
        RealmResults<TaskData> result = realm.where(TaskData.class).findAll();
        if(result.size() > 0) {
    ...
    {
        final TaskData taskVal = new TaskData(task,priority, scheduleTime,isDone,scheduleTime);
        taskVal.taskId = new Date().toString();

      realm.executeTransaction(new Realm.Transaction() {
          @Override
          public void execute(Realm realm) {
              realm.insert(taskVal);
          }
      });
    ...
    public void updateIsDone(final String taskId)
    {
        ...
        Realm.getDefaultInstance().executeTransaction(new Realm.Transaction() {
            ...
            public void execute(Realm realm) {
                ...
                final RealmResults<TaskData> tasks = realm.where(TaskData.class).equalTo("taskId", taskId).findAll();
                tasks.get(0).isDone = !tasks.get(0).isDone;
                tasks.get(0).scheduleTime = new Date().toString();

                realm.insertOrUpdate(tasks.get(0));

                String message = tasks.get(0).isDone ?"Marked "+ tasks.get(0).task +" as Completed":"Marked "+ tasks.get(0).task +" as pending";
